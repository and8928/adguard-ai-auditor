...
async function loadCurrentFilters() {
  ...
  try {
    ...
    if (data.clean_rules_objects && Object.keys(data.clean_rules_objects).length > 0) {
      let html = `<div style="font-size:0.78rem; color:var(--text-muted); margin-bottom:8px;">
        ${t('filters.total', data.stats?.total_clean || Object.keys(data.clean_rules_objects).length)}
      </div>`;
      html += '<div style="max-height:350px; overflow-y:auto;">';
      html += '<table class="results-table"><thead><tr>';
      html += `<th>${t('table.domain')}</th><th>${t('table.type')}</th><th>${t('table.modifiers')}</th>`;
      html += '</tr></thead><tbody>';

      for (const [domain, rule] of Object.entries(data.clean_rules_objects)) {
        const typeLabel = rule.is_exception ? '🟢 Exception' : '🛑 Block';
